...
class Wgan:
    ...
    def __init__(self, input_shape, batch_size):
        ...
        if os.path.isfile("trained_parameters/wgan/generator/ckpt_generator.index"):
            self.generator.load_weights("trained_parameters/wgan/generator/ckpt_generator")
        self.discriminaor = self.make_discriminaor(input_shape)
        if os.path.isfile("trained_parameters/wgan/discriminaor/ckpt_discriminaor.index"):
            self.discriminaor.load_weights("trained_parameters/wgan/discriminaor/ckpt_discriminaor")
        # optimizer
    ...
    def make_generator(self, input_shape, output_shape):
        ...
        return tf.keras.Sequential([
            layers.Dense(width*height*256, use_bias=False, input_shape=input_shape),
            layers.BatchNormalization(),
            layers.LeakyReLU(),
            layers.Reshape((width, height, 256)),
            ...
            layers.LeakyReLU(),
            layers.Conv2DTranspose(
                output_shape[3], 5, strides=2, padding='same', use_bias=False, activation='tanh')
        ])
    ...
    def train(self, ds, log=False):
        # g_loss, d_loss = self.train_step(tf.convert_to_tensor(list(map(lambda i: i.tolist(), images))))
        # g_loss, d_loss = self.train_step((np.array(images) - 127.5) / 127.5)
        # if log:
        ...
        for step in range(10000):
            ...
            if (step % 20 == 0):
                self.generator.save_weights("trained_parameters/wgan/generator/ckpt_generator")
                self.discriminaor.save_weights("trained_parameters/wgan/discriminaor/ckpt_discriminaor")
                cv2.imshow('input', np.array(images * 127.5 + 127.5).astype(np.uint8)[0])
                cv2.imshow('input', np.array(self.generator(test_z, training=True) * 127.5 + 127.5).astype(np.uint8)[5])
                cv2.waitKey(1)
                print('d_loss: {}'.format(d_loss))
                print('g_loss: {}'.format(g_loss))